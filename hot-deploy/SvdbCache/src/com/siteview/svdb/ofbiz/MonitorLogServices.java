...
//	}
	...
	public static Map<String, Object> insertLogData(DispatchContext dctx,
		...
		Delegator delegator = dctx.getDelegator();		
		String entityIndex = sdf.format(new Date());// table index
		String logId = delegator.getNextSeqId("OperationAttributeLog"+entityIndex);		
		...
		GenericValue log = delegator.makeValue("OperationAttributeLog"+entityIndex, fields);
		delegator.create(log);
		if (response.get("responseMessage").toString().equals("success")) {
			Debug.logInfo("/******插入一条日志数据到数据库表  OperationAttributeLog"+entityIndex+" 中操作成功,日志ID: "+logId+" ******/", module);
		}else{
			Debug.logInfo("/******插入一条日志数据到数据库表  OperationAttributeLog"+entityIndex+" 中操作失败,日志ID: "+logId+" ******/", module);
		}
		return response;
	}
	/**
	...
	public static Map<String, Object> queryLogData(DispatchContext dctx,
		...
		entityConditionList.add(EntityCondition.makeCondition(
				"logTime", EntityOperator.BETWEEN, UtilMisc.toList(
						startdate, enddate)));
		...
		for (GenericValue genericValue : result) {
			...
			String reportCreateTime = genericValue.get("logTime")
					.toString();
			String reportDataValue = genericValue.get("description")
					.toString();
